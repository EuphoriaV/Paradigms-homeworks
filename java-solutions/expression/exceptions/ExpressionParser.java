    ...
    private MultyExpression parseExpression(int priority) {
        if (priority == 0) {
            ...
            while (inPriority(priority)) {
                ...
                res = switch (cur) {
                    case ADD -> new CheckedAdd(res, second);
                    case SUB -> new CheckedSubtract(res, second);
                    case MUL -> new CheckedMultiply(res, second);
                    case DIV -> new CheckedDivide(res, second);
                    case MIN -> new Min(res, second);
                    case MAX -> new Max(res, second);
